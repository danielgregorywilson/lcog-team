...
const mutation: MutationTree<UserStateInterface> = {
  ...
  userSuccess: (state, resp: {data: EmployeeRetrieve}) => {
    ...
    Vue.prototype.$cookies.set('can_view_mow_routes', resp.data.can_view_mow_routes)
    Vue.prototype.$cookies.set('can_manage_mow_stops', resp.data.can_manage_mow_stops)
  },
  userError: (state) => {
    state.status = 'error'
  },
  authLogout: (state) => {
    // Clean up state
    state.profile = {
      employee_pk: -1, username: '', email: '', name: '', is_manager: false, has_manager: false, is_eligible_for_telework_application: false, can_view_seating_charts: false,
      can_edit_seating_charts: false, is_upper_manager: false, is_hr_manager: false, is_executive_director: false, viewed_security_message: false, prs_can_view: [], notes_can_view: [],
      telework_applications_can_view: [], time_off_requests_can_view: [], next_to_sign_prs: '', workflow_roles: [], can_view_mow_routes: false
    }
    ...
    Vue.prototype.$cookies.remove('can_view_mow_routes')
    Vue.prototype.$cookies.remove('can_manage_mow_stops')
  }
};
